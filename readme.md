...
С использованием этих пакетов дело обстоит несколько сложнее. Нужно установить `biblatex`, `biblatex-gost` и `biber` (про установку данных пакетов можно будет прочесть ниже. Стилевой файл подключается так:
...
Из текстов транскрипций удаляются номера страниц, поскольку транскрипция не является и не должна являться полноценной заменой бумажной книги или её факсимильной копии.

Файлы транскрипций находятся в кодировке utf-8 (with byte order mark).
